(() => {
  ...
  class Tear {
    ...
    draw(ctx,time){
      if(!this.done){
        ...
        if(this.trait.id!=='plain'){ctx.fillStyle=this.trait.color;ctx.font='12px Georgia, Noto Serif KR, serif';ctx.textAlign='center';ctx.fillText(`${this.trait.glyph} ${this.trait.nameKo} · ${this.trait.nameEn}`,this.x,this.y-24);}
      }else{
  ...
  class Game {
    ...
    drawHud(ctx){
      ...
      const charmHudLine=this.charmHudLine();if(charmHudLine){ctx.fillStyle='rgba(231,221,200,.66)';ctx.font='12px Georgia, Noto Serif KR, serif';ctx.fillText(charmHudLine,42,126);}
