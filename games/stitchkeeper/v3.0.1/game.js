(() => {
  ...
  const UI_TYPOGRAPHY=Object.freeze({traitLabelFontPx:16,charmHudFontPx:16});
  const TRAIT_LABELS=Object.freeze({thorn:'✕ 가시 · THORN',ward:'◎ 수호 · WARD',moon:'◒ 달빛 · MOON'});
  ...
  class Tear {
    constructor(spec,traitId='plain'){this.id=spec.id;this.x=spec.x;this.y=spec.y;this.len=spec.length;this.angle=spec.angle;this.a={...spec.a};this.b={...spec.b};this.path=spec.path.map(point=>({...point}));this.trait=Rules.TEAR_TRAIT_BY_ID[traitId]||Rules.TEAR_TRAIT_BY_ID.plain;this.done=false;this.flash=0;}
    draw(ctx,time){
      if(!this.done){
        ...
        for(const point of [this.a,this.b]){const radius=7+Math.sin(time*4+this.id)*2;ctx.strokeStyle=this.trait.color;ctx.globalAlpha=.65;ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(point.x,point.y,radius,0,TAU);ctx.stroke();ctx.globalAlpha=1;ctx.fillStyle=C.paper;ctx.beginPath();ctx.arc(point.x,point.y,2.8,0,TAU);ctx.fill();}
        if(this.trait.id!=='plain'){ctx.fillStyle=this.trait.color;ctx.font=`${UI_TYPOGRAPHY.traitLabelFontPx}px Georgia, Noto Serif KR, serif`;ctx.textAlign='center';ctx.fillText(TRAIT_LABELS[this.trait.id],this.x,this.y-24);}
      }else{
  ...
  class Moth {
    ...
    update(dt,game){
      ...
      if(game.active&&this.stun<=0&&Rules.isMothTouchingThread(this,game.active.points,Rules.MOTH_CONTACT_RADIUS)){this.bite+=dt;if(this.bite>game.effectiveTuning().cutContactSeconds*game.active.tear.trait.cutContactMultiplier)game.cutThread(this);}else this.bite=Math.max(0,this.bite-dt*2);
    }
    draw(ctx,cutTime){
  ...
  class InputKeeper {
    constructor(game,canvas){
      ...
      window.addEventListener('keydown',event=>{const interactive=event.target instanceof Element&&event.target.closest('button,a,input,select,textarea,[role="button"]');if(event.code==='Space'&&!interactive){event.preventDefault();game.pulse();}if(event.code==='KeyP'||event.code==='Escape')game.togglePause();if(event.code==='Enter'&&!interactive&&['title','gameover','victory'].includes(game.state)){event.preventDefault();game.start();}});
    }
  ...
  class Game {
    constructor(canvas){
      ...
      this.state='title';this.selectedDifficultyId=Rules.DEFAULT_DIFFICULTY_ID;this.profile=Rules.DIFFICULTY_PROFILES[this.selectedDifficultyId];this.nightIndex=0;this.runSeed='------';this.runPlan=[];this.ownedCharms=[];this.draftChoices=Object.freeze([]);this.lastSeamResult=null;this.charmEffects=Rules.aggregateCharmEffects([]);this.elapsed=0;this.nightTime=0;this.score=0;this.combo=0;this.integrity=3;this.thread=100;this.pulses=3;this.pulseWave=0;this.pulseCooldown=0;this.tears=[];this.moths=[];this.marks=[];this.active=null;this.pointer={x:640,y:400,down:false};this.message='';this.messageLife=0;this.shake=0;this.last=performance.now();this.tutorial=true;
    ...
    get maxThread(){return 100+this.charmEffects.maxThreadBonus;}
    get maxPulses(){return 3+this.charmEffects.maxPulsesBonus;}
    get maxIntegrity(){return 3+this.charmEffects.maxIntegrityBonus;}
    get eclipse(){return Rules.getEclipseModifiers(this.nightIndex+1);}
    highScore(){return Number(localStorage.getItem(`stitchkeeper.high.${this.selectedDifficultyId}`)||0);}
    effectiveTuning(){const scale=1+this.nightIndex*.035,e=this.eclipse,c=this.charmEffects;return{idleSpeed:Math.round(this.profile.idleSpeed*scale*e.speedMultiplier*c.mothSpeedMultiplier),activeSpeed:Math.round(this.profile.activeSpeed*scale*e.speedMultiplier*c.mothSpeedMultiplier),idleAcceleration:Math.round(this.profile.idleAcceleration*scale*e.accelerationMultiplier*c.mothAccelerationMultiplier),activeAcceleration:Math.round(this.profile.activeAcceleration*scale*e.accelerationMultiplier*c.mothAccelerationMultiplier),reactionDelay:this.profile.reactionDelay,cutContactSeconds:this.profile.cutContactSeconds*c.cutContactMultiplier,pulseStunSeconds:this.profile.pulseStunSeconds};}
    ...
    setDifficulty(id){if(!Rules.DIFFICULTY_PROFILES[id]||['play','paused','draft'].includes(this.state))return false;this.selectedDifficultyId=id;this.profile=Rules.DIFFICULTY_PROFILES[id];return true;}
    ...
    start(){
      this.audio.wake();this.profile=Rules.DIFFICULTY_PROFILES[this.selectedDifficultyId];this.runSeed=this.freshSeed();this.runPlan=Rules.createRunPlan(this.runSeed);this.ownedCharms=[];this.draftChoices=Object.freeze([]);this.charmEffects=Rules.aggregateCharmEffects([]);this.score=0;this.combo=0;this.integrity=3;this.thread=100;this.pulses=3;this.nightIndex=0;this.tutorial=true;hideCharmDraft();this.setupNight();this.state='play';setSelectionVisibility(false);this.canvas.focus();this.announce(`${this.profile.labelKo}. Night one of twelve.`);
    }
    setupNight(){
      const night=this.night,selection=this.selection,traitIds=Rules.assignTearTraits(this.runSeed,night.number,night.tears);this.lastSeamResult=null;this.nightTime=night.timeSeconds+this.charmEffects.subsequentNightSeconds;this.tears=Rules.generateTearSpecs(selection.patternId,selection.layoutSeed,night.tears).map((spec,index)=>new Tear(spec,traitIds[index]));this.moths=[];this.active=null;
      const count=Math.max(1,Math.round(night.moths*this.profile.mothScale)+this.eclipse.mothBonus),random=Rules.seededRandom(`${this.runSeed}:moths:${this.nightIndex}`);
    ...
    pointerMove(x,y){
      ...
      if(!active.invalid&&!Rules.isPointInTearCorridor({x,y},active.tear.path,Rules.TEAR_CORRIDOR_RADIUS*active.tear.trait.corridorMultiplier+this.charmEffects.corridorBonus)){active.invalid=true;active.tear.flash=1;this.message='찢어진 선 가까이 꿰매세요';this.messageLife=1.2;this.audio.fail();}
    ...
    completeSeam(){
      if(!this.active)return;if(this.active.invalid){this.cancelSeam('찢어진 선을 벗어난 실이에요');return;}const tear=this.active.tear,trait=tear.trait,tuning=this.effectiveTuning(),corridorRadius=Rules.TEAR_CORRIDOR_RADIUS*trait.corridorMultiplier+this.charmEffects.corridorBonus,cutContactSeconds=tuning.cutContactSeconds*trait.cutContactMultiplier,pulsesBefore=this.pulses;tear.done=true;const efficiency=Math.max(0,1-Math.abs(this.active.length-tear.len)/(tear.len*1.6)),base=300+efficiency*250+this.combo*75,scoreAwarded=Math.round(base*this.profile.scoreMultiplier*trait.scoreMultiplier*this.eclipse.scoreMultiplier*this.charmEffects.scoreMultiplier);this.score+=scoreAwarded;this.combo++;this.thread=clamp(this.thread+18,0,this.maxThread);if(trait.restorePulses)this.pulses=clamp(this.pulses+trait.restorePulses,0,this.maxPulses);this.active=null;if(trait.wardRadius)this.ward(tear.x,tear.y,trait.wardRadius);if(this.charmEffects.seamWardRadius)this.ward(tear.x,tear.y,this.charmEffects.seamWardRadius);this.lastSeamResult=Object.freeze({traitId:trait.id,scoreAwarded,traitScoreFactor:trait.scoreMultiplier,eclipseScoreFactor:this.eclipse.scoreMultiplier,charmScoreFactor:this.charmEffects.scoreMultiplier,difficultyScoreFactor:this.profile.scoreMultiplier,corridorRadius,cutContactSeconds,pulsesRestored:this.pulses-pulsesBefore,wardRadius:trait.wardRadius||this.charmEffects.seamWardRadius||0});this.audio.success();this.message=trait.id==='moon'?'달빛 솔기 · 골무 +1':trait.id==='ward'?'수호 솔기 · WARD':this.combo>1?`${this.combo}겹 매듭 · COMBO`:'매듭을 지었습니다';this.messageLife=1.4;for(let i=0;i<18;i++)this.emit(tear.x,tear.y,(textureHash(i+tear.id)-.5)*80,(textureHash(i*3+tear.id)-.5)*80,.7,0);
    }
    ...
    ward(x,y,radius,award=false){this.pulseWave=1;this.pointer.x=x;this.pointer.y=y;for(const moth of this.moths)if(distanceSquared(moth.x,moth.y,x,y)<radius*radius){moth.stun=this.effectiveTuning().pulseStunSeconds;const distance=Math.hypot(moth.x-x,moth.y-y)||1;moth.vx+=(moth.x-x)/distance*230;moth.vy+=(moth.y-y)/distance*230;if(award)this.score+=Math.round(25*this.profile.scoreMultiplier*this.eclipse.scoreMultiplier*this.charmEffects.scoreMultiplier);}this.audio.pulse();for(let i=0;i<22;i++){const angle=i/22*TAU;this.emit(x,y,Math.cos(angle)*130,Math.sin(angle)*130,.6,0);}}
    pulse(x=this.pointer.x,y=this.pointer.y){if(this.state!=='play'||this.pulseCooldown>0||this.pulses<=0)return;this.audio.wake();this.pulses--;this.pulseCooldown=.55;this.ward(x,y,185,true);}
    emit(x,y,vx,vy,life,kind){this.marks[this.markCursor++%this.marks.length].set(x,y,vx,vy,life,kind);}
    chooseCharm(id){if(this.state!=='draft'||this.ownedCharms.includes(id)||!this.draftChoices.includes(id))return false;this.ownedCharms.push(id);this.draftChoices=Object.freeze([]);this.charmEffects=Rules.aggregateCharmEffects(this.ownedCharms);if(id==='iron-spool')this.thread=clamp(this.thread+35,0,this.maxThread);if(id==='moon-thimble'&&this.charmEffects.refillPulsesOnPickup)this.pulses=this.maxPulses;if(id==='menders-heart')this.integrity=clamp(this.integrity+1,0,this.maxIntegrity);hideCharmDraft();this.nightIndex++;this.pulses=Math.min(this.maxPulses,this.pulses+1);this.setupNight();this.state='play';this.canvas.focus();const charm=Rules.CHARM_CATALOG.find(item=>item.id===id);this.announce(`${charm.nameKo}, ${charm.nameEn}. Night ${this.nightIndex+1}.`);return true;}
    ...
    finish(win){this.state=win?'victory':'gameover';this.draftChoices=Object.freeze([]);hideCharmDraft();const high=Math.max(this.highScore(),this.score);localStorage.setItem(`stitchkeeper.high.${this.selectedDifficultyId}`,String(high));setSelectionVisibility(true,win?'STITCH ANOTHER RUN':'TRY THE NIGHT AGAIN');this.announce(win?'Dawn restored. Victory.':'The night unraveled. Game over.');}
    advanceNight(){const cleared=this.nightIndex+1;if(Rules.CHARM_DRAFT_AFTER_NIGHTS.includes(cleared)){const draft=Rules.createCharmDraft(this.runSeed,cleared,this.ownedCharms);this.draftChoices=Object.freeze(draft.map(charm=>charm.id));this.state='draft';showCharmDraft(draft);this.announce('한 땀을 고르세요. Choose a charm.');return;}if(this.nightIndex<Rules.TOTAL_NIGHTS-1){this.nightIndex++;this.pulses=Math.min(this.maxPulses,this.pulses+1);this.setupNight();this.announce(`Night ${this.nightIndex+1} of twelve.`);}else this.finish(true);}
    update(dt){
      ...
      if(this.state!=='play')return;this.nightTime-=dt;this.thread=clamp(this.thread+dt*1.35,0,this.maxThread);for(const moth of this.moths)moth.update(dt,this);
    ...
    drawHud(ctx){
      ...
      const charmHudLine=this.charmHudLine();if(charmHudLine){ctx.fillStyle='rgba(231,221,200,.66)';ctx.font=`${UI_TYPOGRAPHY.charmHudFontPx}px Georgia, Noto Serif KR, serif`;ctx.fillText(charmHudLine,42,126);}
      ...
      ctx.textAlign='right';ctx.fillStyle='rgba(231,221,200,.76)';ctx.font='16px Georgia, serif';ctx.fillText(`THREAD ${Math.ceil(this.thread)} / ${this.maxThread}`,1234,48);ctx.strokeStyle='rgba(231,221,200,.35)';ctx.strokeRect(1054,58,180,10);ctx.fillStyle=this.thread<this.maxThread*.25?C.rust:C.thread;ctx.fillRect(1057,61,174*this.thread/this.maxThread,4);ctx.fillStyle='rgba(231,221,200,.7)';ctx.font='15px Georgia, serif';ctx.fillText(`INTEGRITY  ${'◆'.repeat(this.integrity)}${'◇'.repeat(this.maxIntegrity-this.integrity)}`,1234,91);ctx.fillText(`THIMBLES  ${'●'.repeat(this.pulses)}${'·'.repeat(this.maxPulses-this.pulses)}`,1234,114);if(this.eclipse.isEclipse){ctx.fillStyle=C.rust;ctx.font='bold 14px Georgia, Noto Serif KR, serif';ctx.fillText('월식 · ECLIPSE',1234,139);}
    }
    charmHudLine(){const charms=Rules.CHARM_CATALOG.filter(charm=>this.ownedCharms.includes(charm.id));return charms.length?`부적 · CHARMS  ${charms.map(charm=>`${charm.sigil} ${charm.nameKo}`).join(' / ')}`:'';}
    ...
    render(){
      const ctx=this.ctx;ctx.save();if(this.shake&&!this.reduced)ctx.translate((Math.random()-.5)*8*this.shake,(Math.random()-.5)*8*this.shake);this.paper(ctx);if(this.eclipse.isEclipse&&this.state!=='title'){const shade=ctx.createRadialGradient(640,360,210,640,360,690);shade.addColorStop(0,'rgba(125,84,59,0)');shade.addColorStop(1,'rgba(154,79,61,.24)');ctx.fillStyle=shade;ctx.fillRect(0,0,W,H);}if(this.state==='title'){this.drawTitle(ctx);ctx.restore();return;}this.drawHud(ctx);for(const tear of this.tears)tear.draw(ctx,this.elapsed);this.drawActive(ctx);for(const moth of this.moths)moth.draw(ctx,this.effectiveTuning().cutContactSeconds*(this.active?this.active.tear.trait.cutContactMultiplier:1));for(const mark of this.marks)if(mark.life>0)mark.draw(ctx);
  ...
  const canvas=document.querySelector('#game'),game=new Game(canvas),mute=document.querySelector('#mute'),start=document.querySelector('#start'),panel=document.querySelector('#difficulty-panel'),description=document.querySelector('#difficulty-description'),choices=[...document.querySelectorAll('.difficulty-choice')],charmPanel=document.querySelector('#charm-panel'),charmChoices=document.querySelector('#charm-choices');
  function setGameplayFocusability(enabled){canvas.tabIndex=enabled?0:-1;mute.disabled=!enabled;}
  function hideCharmDraft(){charmPanel.hidden=true;charmChoices.replaceChildren();setGameplayFocusability(true);}
  function showCharmDraft(draft){charmChoices.replaceChildren(...draft.map(charm=>{const button=document.createElement('button');button.type='button';button.className='charm-choice';button.dataset.charm=charm.id;button.innerHTML=`<span class="charm-sigil" aria-hidden="true">${charm.sigil}</span><span class="charm-name">${charm.nameKo} · ${charm.nameEn}</span><span class="charm-effect">${charm.effectKo}<br>${charm.effectEn}</span>`;button.addEventListener('click',()=>game.chooseCharm(charm.id));button.addEventListener('keydown',event=>{const buttons=[...charmChoices.querySelectorAll('button')],index=buttons.indexOf(button);if(event.code==='Tab'){if(!event.shiftKey&&index===buttons.length-1){event.preventDefault();buttons[0].focus();}else if(event.shiftKey&&index===0){event.preventDefault();buttons[buttons.length-1].focus();}return;}if(!['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(event.code))return;event.preventDefault();const direction=['ArrowRight','ArrowDown'].includes(event.code)?1:-1;buttons[(index+direction+buttons.length)%buttons.length].focus();});return button;}));setGameplayFocusability(false);charmPanel.hidden=false;charmChoices.querySelector('button').focus();}
  ...
  function debugSnapshot(){
    ...
    return Object.freeze({mode:game.state,difficultyId:game.selectedDifficultyId,difficultyLabel:game.profile.labelKo,night:game.nightIndex+1,totalNights:Rules.TOTAL_NIGHTS,runSeed:game.runSeed,currentPatternId:selection?selection.patternId:null,currentPatternName:selection?`${selection.patternNameKo} · ${selection.patternNameEn}`:null,phase:game.nightIndex+1,score:game.score,combo:game.combo,integrity:game.integrity,maxIntegrity:game.maxIntegrity,thread:Math.round(game.thread),maxThread:game.maxThread,pulses:game.pulses,maxPulses:game.maxPulses,nightTime:game.nightTime,muted:game.audio.muted,isEclipse:game.eclipse.isEclipse,ownedCharms:Object.freeze([...game.ownedCharms]),charmHudLine:game.charmHudLine(),uiTypography:UI_TYPOGRAPHY,draftChoices:Object.freeze([...game.draftChoices]),lastSeamResult:game.lastSeamResult?Object.freeze({...game.lastSeamResult}):null,charmEffects:Object.freeze({...game.charmEffects}),scoreFactors:Object.freeze({difficulty:game.profile.scoreMultiplier,eclipse:game.eclipse.scoreMultiplier,charm:game.charmEffects.scoreMultiplier}),remainingTears:game.tears.filter(tear=>!tear.done).length,effectiveMothTuning:Object.freeze({...tuning}),mothCount:game.moths.length,representativeSpeeds:Object.freeze(game.moths.slice(0,3).map(moth=>Math.round(Math.hypot(moth.vx,moth.vy)*100)/100)),activeSeamInvalid:game.active?game.active.invalid:null,tears:Object.freeze(game.tears.filter(tear=>!tear.done).map(tear=>Object.freeze({id:tear.id,traitId:tear.trait.id,a:point(tear.a),b:point(tear.b),path:Object.freeze(tear.path.map(point))})))});
  }
  window.__STITCHKEEPER__=Object.freeze({snapshot:debugSnapshot,actions:Object.freeze({finishForTest:()=>{if(game.state==='play'||game.state==='paused')game.finish(false);return debugSnapshot();},completeNightForTest:()=>{if(game.state!=='play')return debugSnapshot();for(const tear of game.tears)tear.done=true;game.advanceNight();return debugSnapshot();},chooseCharmForTest:id=>{game.chooseCharm(id);return debugSnapshot();}})});
})();
