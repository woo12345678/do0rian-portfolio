(function (root, factory) {
  ...
  function createGame(opts={}) {
    serial=0; const mode=opts.mode||'classic', requestedPlayers=Math.max(1,Math.min(4,opts.players||2)), players=mode==='classic'||mode==='football'?2:requestedPlayers, s=opts.settings||{};
    const settings={...s};
    ...
    if(mode==='line'){g.board={width:W,height:H,walls:true,orientation:'vertical',targetAxis:'y',targetLine:160};g.throws=s.throws||3;g.attempts=Array.from({length:players},()=>[]);g.objects=Array.from({length:players},(_,t)=>disc(t,520+t*160,620));}
  ...
  function resolve(g,events=[]){events=P.accumulateEvents([],events);g.phase='resolution';g.events=events;
    ...
    if(g.mode==='line'){const o=g.objects.find(o=>o.active&&o.kind==='disc'&&o.team===g.turn);const legal=!!o&&o.y>=g.board.targetLine&&o.y<=H;g.attempts[g.turn].push({legal,distance:legal?Math.round((o.y-g.board.targetLine)*10)/10:null});if(g.attempts.every(a=>a.length>=g.throws)){let best=Infinity,w=null;g.attempts.forEach((a,i)=>a.forEach(v=>{if(v.legal&&v.distance<best){best=v.distance;w=i;}}));g.winner=w;g.phase='finished';return;}o.x=520+g.turn*160;o.y=620;}
    if(g.mode==='golf'){const o=g.objects.find(o=>o.kind==='disc'&&o.team===g.turn);const sunk=events.some(e=>e.type==='cup'&&e.team===g.turn)||(!o?.active||Math.hypot(o.x-g.board.cup.x,o.y-g.board.cup.y)<g.board.cup.r);if(sunk&&o){o.active=false;o.vx=o.vy=0;}const active=g.objects.filter(d=>d.kind==='disc'&&d.active);if(sunk&&!active.length){g.hole++;if(g.hole>=HOLES.length){g.phase='finished';g.winner=g.scores.indexOf(Math.min(...g.scores));return;}g.turn=0;setupHole(g);g.phase='aiming';return;}for(let i=1;i<=g.players;i++){const next=(g.turn+i)%g.players;if(active.some(d=>d.team===next)){if(next<=g.turn)g.round++;g.turn=next;g.phase='aiming';return;}}g.phase='finished';return;}
    ...
    if(g.mode==='chain'){for(const e of events.filter(e=>e.type==='target'))if(e.number===g.nextTarget){g.scores[e.team]++;g.nextTarget++;g.combo++;}else g.combo=0;if(g.nextTarget>5){g.phase='finished';g.winner=g.coop?0:g.scores.indexOf(Math.max(...g.scores));return;}if(g.shot>=g.turnLimit){g.phase='finished';g.winner=null;if(!g.coop){const high=Math.max(...g.scores),leaders=g.scores.map((score,index)=>score===high?index:null).filter(index=>index!==null);if(leaders.length===1)g.winner=leaders[0];}return;}}
    advance(g);
  }
  return {MODES:['classic','football','line','golf','coop','royal','chain'],FORMATIONS,FIELD_GEOMETRY,formationPoints,HOLES,setupHole,createGame,eligibleObjects,markShot,beginGoblinCounter,resolve};
});
